...
export const UserMenu = ({ isMobileMode }: UserMenuProps) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setExportImportDialogOpen(false); // Close dialog after successful import
      } catch (error) {
  ...
  return (
    <>
      ...
      {isMobileMode ? (
        // Mobile authenticated view: Dialog
        <>
          <Button
            variant="outline"
            size="icon"
            onClick={() => {
              ...
              setExportPath(null); // Clear previous path when opening
            }}
          >
          ...
          <Dialog open={exportImportDialogOpen} onOpenChange={setExportImportDialogOpen}>
            <DialogContent mobileFullscreen>
              ...
              <div className="flex flex-col gap-2">
                ...
                {exportPath && (
                  <div className="text-sm text-muted-foreground mt-2 p-2 bg-muted rounded-md break-all">
                    Файл сохранен как: <strong>{exportPath}</strong>
                  </div>
                )}
              ...
              <DialogFooter>
                {/* No specific footer content needed, buttons are in the body */}
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </>
      ) : (
        // Desktop authenticated view: Dropdown Menu
        <DropdownMenu onOpenChange={(open) => {
          if (!open) setExportPath(null); // Clear path when dropdown closes
        }}>
